...
export async function getControlStatus(req, res) {
    ...
    try {
      ...
      for (const control of controls) {
        ...
        const completionStatuses = await CompletionStatus.find({ ...statusQuery }).exec();
        const totalActions = completionStatuses.length;
        const completedActions = completionStatuses.filter(status => status.isCompleted).length;
...
export async function getControlFamilyStatus(req, res) {
    ...
    try {
      ...
      for (const family of controlFamilies) {
        ...
        const controls = await Control.find({ control_Family_Id: family._id }).exec();
        ...
        for (const control of controls) {
          ...
          const completionStatuses = await CompletionStatus.find({ ...statusQuery, isCompleted: true }).exec();
