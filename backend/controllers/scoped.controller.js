...
// Function to get scope name by scope ID
const getScopedNameById = AsyncHandler(async (req, res) => {
  try {
    const { scopeId } = req.params;

    // Find scope by ID
    const scoped = await Scoped.findById(scopeId);

    // Check if the scope was found
    if (!scoped) {
      return res.status(404).json({ message: 'Scope not found' });
    }

    // Return the name of the scope
    res.status(200).json({ name: scoped.name });
  } catch (error) {
    // Handle any errors
    res.status(500).json({ error: 'Error fetching scope name' });
  }
});
