...
#     azure_ips = get_azure_ip_ranges()
#     for prefix in azure_ips:
#         if ip_in_range(ip, prefix['addressPrefixes']):
#             return True
#     return False

# # Google Cloud IP Range Check
# def get_gcloud_ip_ranges():
#     url = 'https://cloud.google.com/compute/docs/regions-zones#network-topology'  # GCP IP Ranges
#     response = requests.get(url)
#     try:
#         response_data = response.json()
#         return response_data.get('prefixes', [])
#     except ValueError:
#         print(f"Error: Received non-JSON response from Google Cloud IP ranges")
#         return []

# def is_gcloud_ip(ip):
    # gcloud_ips = get_gcloud_ip_ranges()
    # for prefix in gcloud_ips:
    #     if ip_in_range(ip, prefix['ip_prefix']):
    #         return True
    # return False
# import sys
...
# input_data = '{"devices": ["192.168.1.1", "192.168.1.41"]}'  # Hardcoded input
...
# # Azure IP Range Check
# def get_azure_ip_ranges():
#     url = 'https://www.microsoft.com/en-us/download/details.aspx?id=56519'  # Azure IP Ranges
#     response = requests.get(url)
