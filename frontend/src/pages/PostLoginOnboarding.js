...
import { checkFormCompletion } from '../api/userApi';
...
const PostLoginOnboarding = () => {
  ...
  useEffect(() => {
    const fetchCompletionStatus = async () => {
      try {
        ...
        const { hasCompletedCompanyForm } = await checkFormCompletion(
          user.data._id
        );
  ...
  return (
    <div className='w-full flex items-center justify-center h-[calc(100vh-(3.5rem+5.5rem))]'>
      ...
      {step === 2 && <CompanyDetailsForm phoneNumber={phoneNumber} otp={otp} />}
    </div>
  );
};
